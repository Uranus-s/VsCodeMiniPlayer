...
- 支持 `.mp4` 和 `.mkv` 视频；MKV 会自动重封装为缓存 MP4 后播放。
...
```text
mini-player-0.0.2.vsix
```

使用命令安装：

```powershell
code --install-extension .\mini-player-0.0.2.vsix
```
...
MKV 文件会通过内置 ffmpeg 自动重封装到扩展缓存目录，不会修改原始文件。当前仍不支持 AVI、MOV、M4V 或 WebM。
...
- Support `.mp4` and `.mkv` videos; MKV files are automatically remuxed to cached MP4 files before playback.
...
MKV files are automatically remuxed with bundled ffmpeg into the extension cache without modifying the original file. AVI, MOV, M4V, and WebM are still unsupported.
